...
//ajout des voisins pour chaque auteur_struct
auteur_courant=graphe->first;

//pour tous les auteurs du graphe
for(int k=0; k<size_graphe; k++)
{
    int size_list_A_a_courant=auteur_courant.size;
    a_deja_mis_dans_voisins_ou_pas flag=pas_mis;

    //pour tout les articles écrit par l'auteur_struct courant
    for(int l=0; l<size_list_A_a_courant; l++)
    {
        //pour tous les auteurs de l'article courant écrit par l'auteur_struct courant
        for(int m=0; m<)
